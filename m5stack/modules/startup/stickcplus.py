...
btnB = Pin(39, Pin.IN)
btnBq = deque([], 5)
# LED: G26 (hat pin; optional indicator; comment out these lines to disable)
...
def start():
    # comment next line to disable G26 indicator LED, then uncomment the line below
    run(gather(Packager.work(), monitor_btn(btnA, btnAq, 800), monitor_btn(btnB, btnBq, 200), bloop(led26q, led26)))
    #run(gather(Packager.work(), monitor_btn(btnA, btnAq, 800), monitor_btn(btnB, btnBq, 200)))
start()
'''
...
class StickCPlus_Startup:
    ...
    def startup(self, ssid: str, pswd: str, timeout: int = 60) -> None:
        if ssid != '':
            ...
            print('Did not attempt to connect to WiFi because SSID is empty')
